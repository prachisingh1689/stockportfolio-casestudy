...
public class ReadXMLFile implements Runnable {
	...
	public void run() {
		...
		while (true) {
			...
			StockSaxManager saxManager = new StockSaxManager();
			try {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					stock.setStock_id(Integer.parseInt(node
							.getChildText("stockid")));
					lateststock.setStock(stock);
					lateststock.setName(node.getChildText("name"));
					lateststock.setPrice(Float.parseFloat(node
							.getChildText("price")));

					saxManager.add(lateststock);
