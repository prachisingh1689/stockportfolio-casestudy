 ...
 * 
 */
public class VariationThreadManager {
	...
	public synchronized Float checkChange(List<XMLStockMaster> xmlstockList,
		...
		for (XMLStockMaster xml : xmlstockList) {
			if (xmlid < xml.getXmlId()) {
				currentPrice = xml.getPrice();
				//logger.info(xml.getTime());
			}
		}
		variation = ((currentPrice - addedPrice) / addedPrice) * MyConstant.HUNDRED;
		return variation;
	...
	public synchronized List<UserStockMaster> getDetails(int userstockid) {
		...
		logger.info("transaction has been successfully committed");
	...
	public synchronized void deleteAlert(int userstockid) {
		...
		UserStockMaster stock = (UserStockMaster) session.load(UserStockMaster.class, userstockid);
