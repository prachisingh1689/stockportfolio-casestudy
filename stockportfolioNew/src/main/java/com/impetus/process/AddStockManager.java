...
public class AddStockManager {
	...
	private static AddStockManager addstockmanager=null;
	private AddStockManager()
	{

	}
	public static AddStockManager getAddStockManagerInstance()
	{
		if(addstockmanager==null)
		{
			addstockmanager=new AddStockManager();
			return addstockmanager;
		}
		else
		{
			return addstockmanager;
	...
	public UserStockMaster delete(int id) {
		...
		UserStockMaster stock = (UserStockMaster) session.load(UserStockMaster.class, id);
		if (null != stock) {
	...
	public List<UserStockMaster> list() {
		...
		int id = (Integer) ActionContext.getContext().getSession().get("user_id");
		logger.info(id);
		try {
